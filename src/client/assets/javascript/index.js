...
async function handleCreateRace() {
	try {
		...
		if (!track_id || !player_id) {
			alert(`A Track and a Player must be select to continue`);
			return;
		}
	...
	return new Promise(resolve => {
		...
		const raceInterval = setInterval(async () => {
			...
			try {
				...
				console.error(`Error in runRace ${error.message}`);
			}
		}, 500);
	})
}
...
async function runCountdown() {
	try {
		...
		return new Promise(resolve => {
			let counterTime = document.getElementById('big-numbers')
			
			// use Javascript's built in setInterval method to count down once per second
			const idCountDown = setInterval(() => {

				// run this DOM manipulation to decrement the countdown for the user
				counterTime.innerHTML = --timer
